    ...
    for epoch in range(n_epochs):
        ...
        if (epoch+1) % n_show == 0:
            ...
            for j in range(4):
                ...
                axs[0, j].set_title(f"Input: {folder_0}")
                axs[1, j].imshow(((y[j].cpu().permute(1,2,0)+1)/2).numpy())
                axs[1, j].set_title(f"Input: {folder_1}")
...
if __name__ == "__main__":
    train_pix2pix(
        r"C:\Users\RODO\Desktop\gan-in-pixel-arts\0_0",     # podmieńcie na swoje ścieżki
        r"C:\Users\RODO\Desktop\gan-in-pixel-arts\1_1"        
    )
